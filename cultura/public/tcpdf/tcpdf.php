...
if(!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 2.3.000 (http://www.tcpdf.org)');
	...
	*/
    ...
    private $n_js;
		...
		*/
		public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0) {
		...
		public function Write($h, $txt, $link='', $fill=0, $align='', $ln=false, $stretch=0) {
			...
			while($i < $nb) {
				...
				if(preg_match("/[\n]/u", $this->unichr($c))) {
					...
					if(preg_match("/[ ]/u", $this->unichr($c))) {
						// update last blank space position
						$sep = $i;
					}
					// update string lenght
					$l += $this->GetStringWidth($this->unichr($c));
					if($l > $wmax) {
		...
		*/
		...
		protected function UTF8StringToArray($str) {
			if(!$this->isunicode) {
				return str_split($str); // string is not in unicode, return an array of chars
			}
